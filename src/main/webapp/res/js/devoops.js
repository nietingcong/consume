	...
	$.ajax({
		...
		success: function(data) {
			if(data.indexOf("login_page") != -1) {
				console.log("当前会话已失效。")
				location.href = "/consume"+"/login.htm";
				return false;
			}
...
$(document).ready(function () {
	...
	$('.main-menu').on('click','a',function(e){
		var parents = $(this).parents('li');//获取当前单击节点对象的所有li父集节点
		var li_dropdown = $(this).closest('li.dropdown');//获取当前单击节点对象的父节点
		var another_lis = $('.main-menu li').not(parents);//其它节点对象
		//移除其它节点下a标签的active、active-parent类
		another_lis.find('a').removeClass('active').removeClass('active-parent');
		if($(this).hasClass('dropdown-toggle')) {//含有子节点
			if($(this).closest('ul.dropdown-menu').length > 0)
				$(this).find('i').remove();//移除i标签
			$(this).addClass('active-parent');
			var current_ul = $(this).next();//获取当前单击节点的下一个元素【子节点集合】
			if(current_ul.is(':visible')) {//如果当前子节点处于展开状态
				if($(this).closest('ul.dropdown-menu').length > 0)
					$(this).prepend('<i class="fa fa-caret-right"></i>');//添加向右三角图标
				li_dropdown.find('ul.dropdown-menu').slideUp('fast');//隐藏当前对象下的子节点集
			} else {
				if($(this).closest('ul.dropdown-menu').length > 0)
					$(this).prepend('<i class="fa fa-caret-down"></i>');//添加向下三角图标
				another_lis.find('ul.dropdown-menu').slideUp('fast');//隐藏其它之前展开的节点集
				another_lis.find('ul.dropdown-menu i')
					.replaceWith('<i class="fa fa-caret-right"></i>');
				li_dropdown.find('> ul.dropdown-menu').slideDown('fast');//显示当前对象下的子节点集
			}
		} else if($(this).closest('li').find('ul').length == 0) {//当前节点下无子节点
			$(this).closest('ul').removeClass('active');//移除同级其它节点的active类
			//隐藏同级节点其它含有子节点的对象
			if($(this).closest('ul.dropdown-menu').length > 0)
				$(this).closest('ul').find('li.dropdown i').replaceWith('<i class="fa fa-caret-right"></i>');
			$(this).closest('ul').find('li.dropdown ul.dropdown-menu').slideUp('fast');
			$(this).addClass('active');//给当前选中节点添加active类
		}
		if($(this).hasClass('ajax-link')) {
			e.preventDefault();
			var url = $(this).attr('href');
